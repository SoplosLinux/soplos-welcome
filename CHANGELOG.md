...
### Fixed
- Depends on `soplos-fontconfig-woff-fix` to prevent Qt applications from crashing in `FcCharSetHasChar` after Chromium 154-based browsers rewrite the user font cache (fontconfig #565).
...
### Added
- **Recommended tab (Office)**: added GenOffice, an open-source AI office suite (Docs, Sheets, Slides, PDF, HTML and Markdown editors). Installs from the `.deb` served at `genoffice.ai/download/linux-deb`, which always redirects to the vendor's latest GitHub release, so the entry never needs to be re-pinned to a version.

### Fixed
- **Recommended tab (silent install failures across the whole tab)**: the script generated by `recommended_tab.py` had no `set -e`, so any failing command (a bad Flatpak ref, a broken download, an `apt` error) was ignored and the script still printed "Operation completed successfully". This masked every other bug below until now — they all failed silently before this fix. `set -e` is now written into every generated script, matching the other five tabs that already had it (Software, Gaming, Drivers, Kernels, Security).
- **Recommended tab (Collabora Office never installed)**: its Flatpak ID was `org.collaboraoffice.CollaboraOffice`, which does not exist on Flathub — verified against Flathub's own API. The real ID is `com.collaboraoffice.Office`.
- **Recommended tab (GeForce NOW installed at system level instead of user level)**: its `install_commands` ran `flatpak remote-add`/`flatpak install` with no `--user`, and since custom `install_commands` always run under `pkexec` (root), the remote and the app landed in root's Flatpak installation instead of the real user's — invisible to Welcome's own install-state check and to every other Flatpak app, which are all installed at user level. Added `--user` to both commands, and `sudo -u $REAL_USER` (resolved from `$PKEXEC_UID`, the same pattern already used by the AppImage entries in this file) so the operations actually target the logged-in user despite the script running as root.
- **Recommended tab (DaVinci Resolve install button stuck forever after cancelling)**: `_on_install_package` marks the package as "installing" before calling `_install_davinci_resolve`, but cancelling either of its first two dialogs (the initial notice, the archive/file picker) returned without ever clearing that mark. The button stayed disabled until Welcome was restarted. Both cancel paths now clear the mark and refresh the UI.
- **Recommended tab (LMMS had no install candidate)**: LMMS is present in Debian stable (trixie) and unstable (sid) but was dropped from testing (forky, Soplos's actual base) — confirmed against Debian's madison archive, not the cached package-search page. Switched from `apt` to its Flathub entry (`io.lmms.LMMS`), which also takes it out of the batch-install `apt` transaction, so a failure there no longer blocks every other package selected in the same batch.
- **Recommended tab (amdgpu_top installed nothing, silently)**: its install script parses the latest GitHub release for a `browser_download_url` ending in `amd64.deb`, but the current release ships two matches (`amdgpu-top_*_amd64.deb` and `amdgpu-top_without_gui_*_amd64.deb`); the un-narrowed `grep` captured both URLs on separate lines, so `wget` received a broken multi-line argument and failed — invisibly, before the `set -e` fix above. Added `grep -v without_gui` to keep only the GUI build.
- **Recommended tab (CPU Power installed but never did anything)**: its `package` key only listed `cpupower-gui` (the GUI), while a second, unused key named `packages` (never read anywhere in the codebase) held `linux-cpupower`, the actual backend the GUI needs to read/set the CPU governor. Moved both into a single `install_commands: apt install -y cpupower-gui linux-cpupower`.
- **Software tab (double password prompt installing Snap Store without Snapd)**: when Snapd wasn't already installed, the generated script ran `pkexec apt install -y snapd ...` and `pkexec snap install ...` as two separate commands, each prompting for authentication. Merged into a single `pkexec bash -c '... && ...'`, one prompt, same fix already applied to Cloudflare WARP. Fixed identically in all three desktop variants (GNOME, Plasma, Xfce).

### Changed
- **Software tab**: removed an emoji from a debug `print()` (not visible in the UI) in all three desktop variants, per project convention.
