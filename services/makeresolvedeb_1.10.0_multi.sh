...
init_deb_data() {
    ...
    createf_directory "${DEB_DIR_DATA}"
    create_directory "${DEB_DIR_DATA}"/DEBIAN
    createf_directory "${RESOLVE_BASE_DIR_DATA}"
    cat > "${DEB_DIR_DATA}"/DEBIAN/control <<EOF
Package: ${DEB_NAME}-data
Version: ${DEB_VERSION}
Section: video
Priority: optional
Architecture: amd64
Maintainer: ${MAINTAINER}
Description: ${RESOLVE_NAME} data made from ${INSTALLER_ARCHIVE} using MakeResolveDeb ${MAKERESOLVEDEB_VERSION}
Conflicts: ${DEB_CONFLICTS}-data
EOF
}

close_deb_data() {
    chmod 644 "${DEB_DIR_DATA}"/DEBIAN/control
}
...
process_21() {
    ...
    copy_object "${UNPACK_DIR}"/plugins "${RESOLVE_BASE_DIR_DATA}"
    copy_object "${UNPACK_DIR}"/Technical\ Documentation "${RESOLVE_BASE_DIR}"
    copy_object "${UNPACK_DIR}"/UI_Resource "${RESOLVE_BASE_DIR}"
    copy_object "${UNPACK_DIR}"/scripts/script.checkfirmware "${RESOLVE_BASE_DIR}"/scripts/
    copy_object "${UNPACK_DIR}"/scripts/script.getlogs.v4 "${RESOLVE_BASE_DIR}"/scripts/
...
DEB_DIR_DATA=./"${DEB_NAME}"-data_"${DEB_VERSION}"_amd64
RESOLVE_BASE_DIR="${DEB_DIR}"/opt/resolve
RESOLVE_BASE_DIR_DATA="${DEB_DIR_DATA}"/opt/resolve
ERRORS=0
...
case $RESOLVE_VERSION in
    ...
    *)
	...
	echo "Trying Resolve 21 conversion process"
	init_deb
	init_deb_data
	installer_extract_native
	process_21
...
if [[ -z "$CI_TEST" ]]; then
    ...
    echo "Creating Debian package(s) (This can take a while. Do not interrupt)"
    ...
    # Create eventual data deb
    if [[ -d "${DEB_DIR_DATA}" ]]; then
	if ! TMPDIR=./tmp fakeroot dpkg-deb -b "${DEB_DIR_DATA}" "${DEB_DIR_DATA}".deb;
	then
	    ERRORS=$((ERRORS+1))
	fi
	remove_directory "${DEB_DIR_DATA}"
    fi
