"""
XFCE Software Management Tab for Soplos Welcome.
Optimized software management interface for XFCE desktop environment.
"""
...
class SoftwareXfceTab(Gtk.Box):
    """
    XFCE-optimized software management tab.
    Features traditional software managers and XFCE-specific integrations.
    """
    ...
    def _init_ui(self):
        """Initialize the XFCE software interface."""
        # Main container - centered
        main_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=15)
        main_box.set_halign(Gtk.Align.CENTER)
        self.pack_start(main_box, True, True, 0)
        
        # Top buttons - Repository management
        self._create_repo_buttons(main_box)
        
        # Software grid (3x2 including repo selector)
    ...
    def _create_software_grid(self, parent):
        """Create the software applications grid."""
        ...
        # Software options for XFCE (5 elements + repo selector = 6 total for 3x2 grid)
        software_options = [
    ...
    def _create_config_buttons(self, parent):
        ...
        # Snap to Whisker Menu (XFCE specific)
        snap_button = Gtk.Button(label=_("Snap to Whisker Menu"))
        snap_button.connect("clicked", self._on_snap_whisker_clicked)
        config_box.pack_start(snap_button, True, True, 0)
    ...
    def _create_repo_selector_column(self, parent):
        ...
        # Description
        description = Gtk.Label(label=_("Manage additional repositories and software sources for your Soplos system."))
    ...
    def _on_update_clicked(self, widget):
        """Update package repositories and check for upgrades."""
        # First update repositories
        script_content = "pkexec apt update"
        
        # Use a custom callback to check for updates after apt update finishes
    ...
    def _on_upgrade_clicked(self, widget):
        ...
        script_content = "pkexec apt upgrade -y"
        self._create_and_run_script(script_content, "upgrade-system.sh")
    ...
    def _on_flathub_clicked(self, widget):
        ...
        self._create_and_run_script(script_content, "add-flathub.sh")
    
    def _on_snap_whisker_clicked(self, widget):
        """Integrate Snap packages with XFCE Whisker Menu."""
        script_content = """
# Install snapd if not installed
pkexec apt install -y snapd
# Create symlink for Whisker Menu integration
pkexec ln -sf /var/lib/snapd/desktop/applications /usr/share/applications/snap
# Update desktop database
pkexec update-desktop-database
"""
        self._create_and_run_script(script_content, "snap-whisker.sh")
