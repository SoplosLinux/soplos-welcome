...
class KernelsTab(Gtk.Box):
    ...
    def _create_ui(self):
        ...
        title = Gtk.Label()
        title.set_markup(f"<span size='20000' weight='bold'>{_('Kernel Management')}</span>")
        title.set_halign(Gtk.Align.START)
        header_box.pack_start(title, False, False, 0)
        
        subtitle = Gtk.Label()
        subtitle.set_markup(f"<span size='12000'>{_('Manage system kernels and updates')}</span>")
        ...
        placeholder.set_markup(f"<span size='14000'>{_('Kernel management functionality coming soon...')}</span>")
